...
# 读取normal_data数据，并将其命名为payload，frac=0.1表示从数据集中随机抽取10%的数据
normal_data = pd.read_csv("deep-xss/dmzo_normal.csv", names=["payload"]).sample(
    frac=0.1
)
# 读取xssed_data数据，并将其命名为payload，frac=0.1表示从数据集中随机抽取10%的数据
xssed_data = pd.read_csv("deep-xss/xssed.csv", names=["payload"]).sample(frac=0.1)
...
# embeddings.similar_by_word("alert(", 10)
...
# 保存 Tokenizer 对象
with open("xss-models/tokenizer.pickle", "wb") as file:
...
os.makedirs(save_dir, exist_ok=True)
# 保存模型
lstm_model.save(os.path.join(save_dir, "trained_xssdec_model.h5"))
# 进行预测
