...
# 导入模型
model = tf.keras.models.load_model("sql-models/trained_sqldec_model.h5")

# 导入tokenizer对象
with open("sql-models/tokenizer.pickle", "rb") as file:
...
def predict_single_sentence(sentence):
    ...
    # 进行填充
    sequence = pad_sequences(sequence, maxlen=109)  # X是训练时的输入序列长度
...
for sentence in sentences:
    ...
    if prediction == 0:
        output = f"{Fore.RED}{sentence}{Style.RESET_ALL} may not sql inject, probability of sql inject {Fore.RED}{str(probability[0][0])}"
        print(output)
    else:
        output = f"{Fore.RED}{sentence}{Style.RESET_ALL} may be sql inject, probability of sql inject {Fore.RED}{str(probability[0][0])}"
        print(output)
