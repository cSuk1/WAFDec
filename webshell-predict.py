...
# 导入模型
model = tf.keras.models.load_model('models/trained_xssdec_model.h5')
...
def predict_single_sentence(sentence):
    ...
    processed_sentence = [word if word in tokenizer.word_index else 'UNK' for word in processed_sentence]  # 根据词汇表替换词汇

    # 将预处理后的语句转换为序列
    sequence = tokenizer.texts_to_sequences([processed_sentence])

    # 进行填充
    sequence = pad_sequences(sequence, maxlen=369)  # X是训练时的输入序列长度
...
# 需要预测的一些句子
sentences = ["<script>alert('xss')</script>", "hello world","onclick=alert('xss')", 
             "javascript:alert('xss')", "<script>document.cookies</script>",
             "?num=<img src=x onerror=alert('XSS')>", "<script src='https://www.example.com/test.js'>",
             "onclick=", "woc", "this is a document", "i like cookies"]
